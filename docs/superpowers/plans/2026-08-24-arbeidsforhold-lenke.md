...
**Architecture:** Den eksisterende `Arbeidsforhold`-bolken beholdes som navigasjonspunkt, men blir en statisk presentasjonskomponent uten store- eller API-avhengigheter. Destinasjonen bruker `VITE_ARBEIDSFORHOLD_URL` når verdien er ikke-tom, mens tom/manglende verdi faller tilbake til `https://www.nav.no/aa-registeret/arbeidsforhold`. Alle interne arbeidsforhold-ruter fjernes, tidligere locale-kvalifiserte stier faller gjennom til standard 404-side, og catch-all rutes som `${basePathWithLanguage}/*` for å unngå transient 404 under locale-innsetting. Hele frontendintegrasjonen mot `@navikt/arbeidsforhold` fjernes.
...
- Bruk ikke-tom `VITE_ARBEIDSFORHOLD_URL` som overstyring av canonical URL.
- Bruk `https://www.nav.no/aa-registeret/arbeidsforhold` når `VITE_ARBEIDSFORHOLD_URL` er tom eller mangler.
- Åpne lenken i samme fane.
- Fjern alle interne `/arbeidsforhold`- og `/arbeidsforhold/:id`-ruter; tidligere locale-kvalifiserte stier skal falle gjennom til standard 404-side.
- Scope catch-all-ruten til `${basePathWithLanguage}/*` for å unngå transient 404 når locale settes inn.
...
- Create `src/__tests__/App.test.tsx`: verifies locale-løs redirect uten transient 404 og standard 404 for tidligere arbeidsforholdstier.
...
const Arbeidsforhold = () => {
    const arbeidsforholdUrl = import.meta.env.VITE_ARBEIDSFORHOLD_URL?.trim() || DEFAULT_ARBEIDSFORHOLD_URL;
...
- [ ] **Step 7: Add fallback coverage for empty environment variable**

In `src/__tests__/forside/Arbeidsforhold.test.tsx`, add a second test that stubs `VITE_ARBEIDSFORHOLD_URL` to an empty/blank value and verifies that lenken fortsatt peker til canonical fallback:

```tsx
it('should use canonical fallback when VITE_ARBEIDSFORHOLD_URL is empty', () => {
    ...
    const link = screen.getByRole('link', { name: 'Se dine arbeidsforhold i Aa-registeret' });
    expect(link).toHaveAttribute('href', 'https://www.nav.no/aa-registeret/arbeidsforhold');
});
```
...
- [ ] **Step 8: Commit the user-facing replacement**
...
Expected: one commit containing the tested UI, copy changes and fallback coverage.
...
Delete the two route declarations for `${basePathWithLanguage}/arbeidsforhold` and `${basePathWithLanguage}/arbeidsforhold/:id` completely (no replacement `Navigate` route).
...
- `/person/personopplysninger` redirects to `/person/personopplysninger/nb/`
- redirect happens without rendering transient 404 (`expect(rendered404).toBe(false)`)
- `${basePath}/nb/arbeidsforhold` renders standard 404
- `${basePath}/nb/arbeidsforhold/123` renders standard 404
...
```bash
if rg -n "@navikt/arbeidsforhold|detaljert-arbeidsforhold|initLocalArbeidsforholdMock|DetaljertArbeidsforhold"   src package.json package-lock.json; then
  echo "Fant rester av den gamle arbeidsforhold-integrasjonen"
  exit 1
fi
if rg -n "path=.*arbeidsforhold|to=.*#arbeidsforhold" src/App.tsx; then
  echo "Fant rester av intern arbeidsforhold-ruting"
  exit 1
fi
```
...
```bash
git add src/App.tsx src/__tests__/App.test.tsx src/index.less src/__tests__/forside/Arbeidsforhold.test.tsx   src/text/nb.ts src/text/nn.ts src/text/en.ts package.json package-lock.json
git add -u src/pages/detaljert-arbeidsforhold
git commit -m "chore: fjern arbeidsforhold-integrasjonen"   -m "Co-authored-by: Copilot <223556219+Copilot@users.noreply.github.com>"
```

Expected: one commit removing only the obsolete frontend integration and internal arbeidsforhold routes.
...
- [ ] **Step 1: Run focused App and Arbeidsforhold tests**
...
Expected: PASS for locale-løs redirect, retired arbeidsforhold routes and CTA fallback behavior.

- [ ] **Step 2: Run the complete test suite once**
...
- [ ] **Step 5: Verify route-remnant cleanup and formatting**
...
Expected: ingen route-remnants funnet, og Prettier returnerer code 0.

- [ ] **Step 6: Verify the exact scope**
