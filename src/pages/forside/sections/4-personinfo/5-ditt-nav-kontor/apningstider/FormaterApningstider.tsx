...
const FormaterApningstider = (props: Props) => {
    ...
    const headingId = isSpesielleAapningstider ? 'dittnavkontor.spesielleapningstider' : 'dittnavkontor.apningstider';

    return (
        <table className="apningstider__table">
            <Heading as="caption" size={'xsmall'} level={'3'} className={'apningstider__caption'}>
            ...
            <tbody>
                {apningstider.map((apningstid) => (
                    <Apningstid
                        key={`${headingId} + ${isSpesielleAapningstider ? apningstid.dato : apningstid.dag}`}
