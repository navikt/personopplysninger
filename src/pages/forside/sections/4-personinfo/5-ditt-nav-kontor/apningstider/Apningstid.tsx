...
const Apningstid = (props: Props) => {
    ...
    return apningstid ? (
        <tr>
            <td>{print(isSpesielleAapningstider ? apningstid.dato && dayjs(apningstid.dato).format('DD.MM.YYYY') : apningstid.dag)}</td>
            <td>{apningstid.stengt === 'true' ? 'Stengt' : `${print(apningstid.fra)} - ${print(apningstid.til)}`}</td>
