                    ...
                    >
                        ...
                        {publikumsmottak.map((mottak, id) => (
                            <option key={id} value={id}>
                                {`${print(mottak.poststed).toUpperCase()} - ${print(mottak.gateadresse)} ${print(mottak.husnummer)}${print(
                                    mottak.husbokstav
                ...
                <div className="dittnavkontor__adresser">
                        ...
                        <div className="dittnavkontor__postadresse">
                            ...
                            <BodyShort>
                                {postadresse.type === 'stedsadresse' &&
                                    `${print(postadresse.gatenavn)} ${print(postadresse.husnummer)} ${print(postadresse.husbokstav)}`}
                                {postadresse.type === 'postboksadresse' &&
                                    `${msg({
                                        id: 'dittnavkontor.postboks',
                                    })} ${print(postadresse.postboksnummer)} ${print(postadresse.postboksanlegg)}`}
                            </BodyShort>
                            <BodyShort>
                    ...
                    {valgtMottakId !== -1 && (
                        <div className="dittnavkontor__publikumsmottak">
                            ...
                            <BodyShort>
                                {`${print(publikumsmottak[valgtMottakId].gateadresse)} ${print(publikumsmottak[valgtMottakId].husnummer)}${print(
                                    publikumsmottak[valgtMottakId].husbokstav
                                )}`}
                            </BodyShort>
                            <BodyShort>
                                {`${print(publikumsmottak[valgtMottakId].postnummer)} ${print(publikumsmottak[valgtMottakId].poststed)}`}
                            </BodyShort>
                        </div>
                    )}
                </div>
            </div>
            <div>{valgtMottakId !== -1 ? <Apningstider publikumsmottak={publikumsmottak} valgtMottakId={valgtMottakId} /> : <hr />}</div>
