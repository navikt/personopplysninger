...
const MedlHistorikkView = (props: Props) => {
    ...
    const medMedlemskap = perioder.filter((periode) => periode.medlem && !periode.studieinformasjon);
    ...
    const ingenPerioder = medMedlemskap.length === 0 && utenMedlemskap.length === 0 && fraLanekassen.length === 0;
    ...
    return (
        <div className={styles.tabsInnhold}>
            ...
            <div className={styles.space}>
                <BodyLong>
                    <FormattedMessage
                        ...
                        values={{
                            lenkeTilFolketrygden: (text) => <Link href={lenkerTilFolketrygden[locale]}>{text}</Link>,
                        }}
                    />
                </BodyLong>
            </div>
            ...
            {medMedlemskap.length > 0 ? (
                <Panel tittelId={"medl.medlemskap.med.tittel"} tittelIdIngress={"medl.medlemskap.med.ingress"} perioder={medMedlemskap} />
            ) : null}
            {utenMedlemskap.length > 0 ? (
                <Panel tittelId={"medl.medlemskap.uten.tittel"} tittelIdIngress={"medl.medlemskap.uten.ingress"} perioder={utenMedlemskap} />
            ) : null}
...
const Checkbox = (props: { text: string }) => (
    <div className={styles.checkbox}>
        <CheckmarkCircleIcon className={styles.kalender} aria-hidden="true" />
        <BodyLong>
